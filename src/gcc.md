...
Здесь мы:
* познакомимся с утилитами `gcc`, `nm`, `ar`;
* научимся собирать из командной строки запускаемый файл, статическую и разделяемую библиотеки;
* объединим компоненты программы в одно запускаемое приложение;
...
Запускаемый файл может быть запущены на исполнение, например, из командной строки.
Обычно, имя такого файла не содержит расширения.
Разделяемым библиотекам дают расширение `.so`.
...
Примерами таких библиотек являются:
* стандартная библиотека Си `libc` (libc(7));
* системная библиотека `linux-vdso` (vdso(7));

Разделяемые библиотеки предоставляют следующие преимущества при их использовании:
* удобство в обновлении программ;
* экономное расходование дискового пространства.
...
Архив -- это специальный файл, группирующий несколькообъектных файлов в один.
В отличие от разделяемой библиотеки, функции содержимое архивного файла помещается внутрь запускаемых файлов и разделяемых библиотек.
